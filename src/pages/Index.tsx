...
export default function Index() {
  ...
  const plans = [
    ...
    },
    {
      ...
      isEnterprise: false,
      color: "#a855f7",
      features: ["মিডিয়াম স্মার্টের সব ফিচার","আনলিমিটেড টেবিল ও স্টাফ","মাল্টি-লোকেশন (৫টি শাখা)","AI রেকমেন্ডেশন (Gemini)","প্রেডিক্টিভ অ্যানালিটিক্স","কাস্টম ব্র্যান্ডিং (লোগো + রং)","প্রায়োরিটি সাপোর্ট ২৪/৭"],
    },
    {
      key: "high_smart_enterprise" as const,
      icon: "🏢",
      name: TIERS.high_smart_enterprise.name_bn,
      desc: "রেস্টুরেন্ট চেইনের জন্য — আনলিমিটেড সব",
      price_monthly: 0,
      price_yearly: 0,
      hot: false,
      isEnterprise: true,
      color: "#f59e0b",
      features: ["হাই স্মার্টের সব ফিচার","আনলিমিটেড রেস্টুরেন্ট গ্রুপ","আনলিমিটেড শাখা","White Label বিকল্প","SLA সাপোর্ট ২৪/৭","কাস্টম POS/ERP ইন্টিগ্রেশন","পূর্ণ API অ্যাক্সেস"],
    },
  ];
    ...
    <div style={{ fontFamily:"'Cormorant Garamond', 'DM Sans', serif", backgroundColor:"#0a0a0a", color:"#e8dcc8", minHeight:"100vh" }}>
          ...
          {!isMobile && (
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              <a href="/menu/demo" style={{ padding:"8px 18px", borderRadius:8, fontSize:13, fontWeight:600, color:"#f5d780", textDecoration:"none", border:"1px solid rgba(201,168,76,0.45)", transition:"all 0.2s" }}
                ...
                onMouseLeave={e => { e.currentTarget.style.borderColor="rgba(201,168,76,0.45)"; e.currentTarget.style.backgroundColor="transparent"; }}>ডেমো</a>
          ...
          <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3,1fr)", gap: isMobile ? 12 : 2 }}>
            {[
              ...
              <Reveal key={i} delay={i * 0.15}>
                <div style={{ padding:"clamp(28px,5vw,44px) clamp(20px,4vw,36px)", backgroundColor:"#141414", border:"1px solid rgba(201,168,76,0.15)", transition:"all 0.4s", position:"relative", overflow:"hidden", borderRadius: isMobile ? 12 : 4 }}
        ...
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
            ...
            {plans.map((p, i) => {
              ...
              return (
                <Reveal key={p.key} delay={i * 0.1}>
                  <div style={{ borderRadius:20, padding:"clamp(24px,4vw,36px) clamp(18px,3vw,28px)", backgroundColor: p.hot ? "#141414" : "#111111", border: p.isEnterprise ? "1px solid rgba(245,158,11,0.4)" : p.hot ? "1px solid rgba(201,168,76,0.5)" : "1px solid rgba(201,168,76,0.15)", boxShadow: p.hot ? "0 0 60px rgba(201,168,76,0.1), inset 0 1px 0 rgba(201,168,76,0.2)" : p.isEnterprise ? "0 0 40px rgba(245,158,11,0.08)" : "none", position:"relative", display:"flex", flexDirection:"column", transition:"all 0.3s", height:"100%" }}
                    ...
                    {p.isEnterprise && <div style={{ position:"absolute", top:-14, left:"50%", transform:"translateX(-50%)", padding:"5px 18px", borderRadius:100, fontSize:11, fontWeight:700, background:"linear-gradient(135deg,#f59e0b,#d97706)", color:"#0a0a0a", letterSpacing:"0.05em", boxShadow:"0 4px 20px rgba(245,158,11,0.4)", whiteSpace:"nowrap", fontFamily:"'DM Sans', sans-serif" }}>🏢 চেইন</div>}

                    <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6, marginTop: (p.hot || p.isEnterprise) ? 8 : 0 }}>
                      <span style={{ fontSize:24 }}>{p.icon}</span>
                      <span style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:22, fontWeight:700, color: p.isEnterprise ? "#f59e0b" : p.hot ? "#f5d780" : "#FFFFFF" }}>{p.name}</span>
                    </div>
                    <div style={{ fontSize:13, color:"rgba(255,255,255,0.55)", marginBottom:20, fontFamily:"'DM Sans', sans-serif" }}>{p.desc}</div>

                    {/* Price */}
                    <div style={{ marginBottom:8, display:"flex", alignItems:"baseline", gap:4 }}>
                      {p.isEnterprise ? (
                        <span style={{ fontSize:"clamp(32px,6vw,44px)", fontWeight:700, fontFamily:"'Cormorant Garamond', serif", lineHeight:1, color:"#f59e0b" }}>কাস্টম</span>
                      ) : (
                        <>
                          <span style={{ fontSize:17, color:"rgba(201,168,76,0.8)", fontFamily:"'DM Sans', sans-serif", fontWeight:600 }}>৳</span>
                          <span style={{ fontSize:"clamp(36px,7vw,52px)", fontWeight:700, fontFamily:"'Cormorant Garamond', serif", lineHeight:1, ...(p.hot ? goldText : { color:"#FFFFFF" }) }}>{price.toLocaleString("bn-BD")}</span>
                          <span style={{ fontSize:14, color:"rgba(255,255,255,0.5)", fontFamily:"'DM Sans', sans-serif" }}>/{billingCycle === "monthly" ? "মাস" : "বছর"}</span>
                        </>
                      )}
                    </div>
                    {!p.isEnterprise && billingCycle === "yearly" && (
                      <div style={{ fontSize:12, color:"#4ade80", fontFamily:"'DM Sans', sans-serif", marginBottom:16 }}>
                        ৳{(p.price_monthly * 12 - p.price_yearly).toLocaleString("bn-BD")} সাশ্রয় হচ্ছে বার্ষিকে
                      </div>
                    )}
                    {p.isEnterprise && (
                      <div style={{ fontSize:12, color:"rgba(245,158,11,0.7)", fontFamily:"'DM Sans', sans-serif", marginBottom:16 }}>
                        যোগাযোগ করুন — আপনার চাহিদা অনুযায়ী মূল্য
                      </div>
                    )}

                    {/* Features */}
                    <div style={{ borderTop:"1px solid rgba(201,168,76,0.15)", paddingTop:20, marginBottom:20, flex:1 }}>
                      {p.features.map((f, fi) => (
                        <div key={fi} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:11 }}>
                          <span style={{ color: p.isEnterprise ? "#f59e0b" : "#f5d780", fontSize:12, fontWeight:800, flexShrink:0 }}>✦</span>
                          <span style={{ fontSize:"clamp(11px,2.5vw,13px)", color:"rgba(255,255,255,0.8)", fontFamily:"'DM Sans', sans-serif" }}>{f}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    {/* CTA Button */}
                    {p.isEnterprise ? (
                      <a href="https://wa.me/8801786130439?text=আমি%20QRManager%20Enterprise%20প্যাকেজ%20সম্পর্কে%20জানতে%20চাই"
                        target="_blank" rel="noopener noreferrer"
                        style={{ display:"block", textAlign:"center", padding:"13px", borderRadius:10, fontSize:13, fontWeight:700, textDecoration:"none", background:"linear-gradient(135deg,#f59e0b,#d97706)", color:"#0a0a0a", boxShadow:"0 6px 24px rgba(245,158,11,0.3)", fontFamily:"'DM Sans', sans-serif", transition:"all 0.2s" }}
          ...
          <Reveal delay={0.35}>
            <p style={{ textAlign:"center", color:"rgba(245,215,128,0.65)", fontSize:"clamp(12px,2.5vw,14px)", marginTop:28, fontFamily:"'DM Sans', sans-serif" }}>
              ✦ সব পেইড প্ল্যানে <span style={{ color:"#f5d780", fontWeight:700 }}>{FREE_TRIAL_DAYS} দিনের ফ্রি ট্রায়াল</span> — কোনো ক্রেডিট কার্ড লাগবে না
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section id="contact" style={{ padding:"clamp(56px,10vw,100px) clamp(16px,5vw,32px)", backgroundColor:"#0d0d0d", borderTop:"1px solid rgba(201,168,76,0.08)" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(260px, 1fr))", gap:20, maxWidth:680, margin:"0 auto" }}>
            {[
              { icon:"💬", title:"WhatsApp", desc:"সবচেয়ে দ্রুত সাড়া পাবেন — সাধারণত ১ ঘন্টায়", value:"+880 1786-130439", href:"https://wa.me/8801786130439?text=আমি%20QRManager%20সম্পর্কে%20জানতে%20চাই", btnText:"WhatsApp করুন", color:"#25D366", bg:"rgba(37,211,102,0.08)", border:"rgba(37,211,102,0.2)" },
              { icon:"📘", title:"Facebook Page", desc:"মেসেজ পাঠান অথবা পোস্টে জিজ্ঞেস করুন", value:"NexCore Technologies", href:"https://facebook.com/nexcoreltd", btnText:"Facebook-এ যান", color:"#1877F2", bg:"rgba(24,119,242,0.08)", border:"rgba(24,119,242,0.2)" },
            ].map((c, i) => (
              <Reveal key={i} delay={i * 0.1}>
                <div style={{ borderRadius:20, padding:"clamp(24px,4vw,32px)", backgroundColor:"#111111", border:`1px solid ${c.border}`, display:"flex", flexDirection:"column", gap:16, transition:"all 0.3s" }}
                  onMouseEnter={e => { e.currentTarget.style.transform="translateY(-4px)"; }}
                  onMouseLeave={e => { e.currentTarget.style.transform="translateY(0)"; }}>
                  <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                    <div style={{ width:48, height:48, borderRadius:14, background:c.bg, border:`1px solid ${c.border}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>{c.icon}</div>
                    <div>
                  ...
                  <div style={{ fontSize:13, fontWeight:600, color:c.color, fontFamily:"monospace", padding:"10px 14px", borderRadius:10, background:c.bg, border:`1px solid ${c.border}` }}>{c.value}</div>
          ...
          <Reveal delay={0.3}>
            <div style={{ marginTop:32, display:"flex", flexWrap:"wrap", gap:16, justifyContent:"center" }}>
              {[{ icon:"📍", text:"সিলেট, বাংলাদেশ" },{ icon:"🕐", text:"সাপোর্ট: সকাল ৯টা — রাত ১০টা" },{ icon:"⚡", text:"সাধারণত ১-২ ঘন্টায় রিপ্লাই" }].map((item, i) => (
      ...
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        @keyframes floatUp { 0% { transform:translateY(0) rotate(0deg); opacity:0; } 8% { opacity:0.07; } 92% { opacity:0.07; } 100% { transform:translateY(-110vh) rotate(360deg); opacity:0; } }
